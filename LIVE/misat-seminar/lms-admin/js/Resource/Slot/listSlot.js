...
function setSlotListData(str) {
    ...
    if(obj.total != 0)
	{
			...
			if (!arrScheduleCode.includes(scheduleCode)){
				...
				<li class="L1 child i${indexNo} ${id}">
					...
					<div class="Tool">
						<ul class="L2s">
							<li class="L2 i1">
								<div class="${scheduleCode} Btn Participant Title">Participant</div>
								<div class="${scheduleCode} Btn Questionnaire Title">Questionnaire</div>
							</li>
						</ul>
					</div>
					<div class="Clear"></div>
		...
		//Menu for Edit
		// <li class="L2 i1">
			// <div class="${id} Btn Edit Title">Edit</div>
		// </li>
		...
		document.getElementById('List').getElementsByClassName('List-Slot')[0].getElementsByClassName('Bodyer')[0].addEventListener('click', e => {
			if (e.target.tagName.toUpperCase() === 'DIV') {
				if (e.target.classList[2] === 'Participant') {
					let id = e.target.classList[0];
					location.href = `../Person/index.html?id=${id}`;
				}
				else if (e.target.classList[2] === 'Questionnaire') {
					let id = e.target.classList[0];
					location.href = `../Questionnaire/index.html?id=${id}`;
				}
			}
		});
	}
