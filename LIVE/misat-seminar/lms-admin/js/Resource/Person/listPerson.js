...
$(document).ready(function(){
	...
	if(loginData==null) {
		...
		if (id){
			...
			getResource(FHIRURL, 'Slot', '?identifier=' + id + "&status=busy&_count=" + loginData.schedule.maxParticipant, FHIRResponseType, 'getAllBusySlotID');
		}
		else
...
let participantArr=[["No.", "Person ID", "Name", "Email", "Institution", "Job Position", "Patient ID", "Account Created Date"]];
function getAllBusySlotID(str){
	...
	if (obj.total == 0)	alert('無資料');
		...
		obj.entry.map((entry, i) => {
			...
			getResource(FHIRURL, 'Appointment', '?slot=Slot/' + slotID + '&_count=' +  + loginData.schedule.maxParticipant, FHIRResponseType, 'getAppointmentBySlotID');
		});
	}
}
...
function getPersonByID(str) {
    ...
    if(index==globalTotalParticipant){
		...
		tablePagination("List-Person", 10);
	}
}