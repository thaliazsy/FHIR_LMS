...
/* PAGE INITIALIZATION */
	...
	function showMyCourse(){
		...
		//for (indexTable=0;indexTable<groupMember.patient.length;indexTable++){											
			...
			document.getElementById("intro").innerHTML+= '<br>User ID: ' + loginData.role[0].patientID; //groupMember.patient[0].patientID;
			//check per schedule
			groupMember.course.forEach(item => {
	...
	function addNewCourseToList(){
		loginData.patient.id= loginData.role[0].patientID;
		getResource(FHIRURL, 'Slot', '?schedule=' + DB.schedule, FHIRResponseType, 'getSlotID');
		let str= '{"resourceType": "Bundle"}';
		createAppointment(str);
	}
