...
$(document).ready(function(){
	...
	if(ret==null) {
		...
		for(item in arr) {
			let memberID=arr[item];
			getResource(FHIRURL, 'PractitionerRole', '?practitioner=' + memberID, FHIRResponseType, 'getPractitionerRoleByPractID');
		};
	}
});
...
function getPractitionerRoleByPractID(obj) {
    let template = [];
    ...
    document.getElementById('List').getElementsByClassName('List-Organization')[0].getElementsByClassName('Bodyer')[0].addEventListener('click', e => {
        if (e.target.tagName.toUpperCase() === 'DIV') {
            if (e.target.classList[2] === 'Detail') {
                ...
                location.href = `../Schedule/index2.html?id=${id}`;
            }
